defmodule AssistantWeb.SettingsUserOAuthControllerTest do
  ...
  setup do
    ...
    original_deployment_mode = Application.get_env(:assistant, :deployment_mode, :cloud)
    ...
    on_exit(fn ->
      ...
      Application.put_env(:assistant, :deployment_mode, original_deployment_mode)
    end)
  ...
  describe "GET /settings_users/auth/google" do
    test "redirects to login in self-hosted mode", %{conn: conn} do
      ...
      assert Phoenix.Flash.get(conn.assigns.flash, :error) ==
               "Google sign in is disabled in self-hosted deployments."
    end
  ...
  describe "GET /settings_users/auth/google/callback" do
    test "rejects callback in self-hosted mode", %{conn: conn} do
      Application.put_env(:assistant, :deployment_mode, :self_hosted)

      conn = get(conn, ~p"/settings_users/auth/google/callback?code=abc&state=expected")
