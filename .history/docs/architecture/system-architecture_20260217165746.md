...
> Revision 5 — Updated 2026-02-17:
> - Memory search simplified: drop pgvector/embeddings, use FTS (tsvector) + tags + structured filters.
>   Semantic vector search deferred as additive future migration.
...
This document defines the architecture for a headless, skills-first AI assistant built in Elixir on the BEAM VM. The system receives user messages from multiple channels (Google Chat, Telegram, WhatsApp, Voice), routes them through an LLM brain (OpenRouter) that selects and executes modular skills organized by domain (email, calendar, CRM, files, markdown). OpenRouter provides LLM chat completions with tool calling and speech-to-text (STT). ElevenLabs provides text-to-speech (TTS) via a thin Req-based HTTP wrapper behind a `TTSClient` behaviour. Skills run in-process as supervised OTP tasks — no external sandbox needed. Data is stored in PostgreSQL with full-text search (tsvector) and structured filters for memory retrieval. The skill interface follows MCP-inspired patterns (self-describing tools with JSON schemas) for future ecosystem compatibility.
...
| OpenRouter | AI Provider | Chat completions with tool calling, STT |
...
#### D. File Version Manager

**Purpose**: Implement the non-destructive file versioning workflow.

**Implementation**: Service module coordinating Drive API calls with database audit trail. File manipulation uses temp directories managed by Briefly.

**Responsibilities**:
- Pull files from Google Drive into temp workspace
- Archive old versions (move to archive folder on Drive, never delete)
- Replace with new versions from workspace
- Maintain full audit trail in database
- Handle failure recovery (restore from archive if upload fails)

**Key modules**:
- `Assistant.Files.VersionManager` — Orchestrates the pull/archive/replace workflow
...
**Implementation**: PostgreSQL via Ecto with full-text search (tsvector/tsquery) and structured filters for memory retrieval.

**Responsibilities**:
- Persist conversation messages and metadata
- Store long-term memories with tags, categories, and full-text search
...
- `Assistant.Memory.Search` — Full-text search + structured filter queries
- `Assistant.Memory.ContextBuilder` — Assemble context for LLM calls (FTS-ranked retrieval weighted by importance)
...
defmodule Assistant.Skills.Drive.UpdateFile do
  ...
  def tool_definition do
    %{
      ...
      description: "Update a file on Google Drive with modifications. Archives the original version automatically.",
      parameters: %{
        "type" => "object",
        "properties" => %{
  ...
  def execute(params, context) do
    # Uses FileVersionManager for safe pull/archive/replace
...
| `:temp_workspace` | Function call with Briefly temp directory | File manipulation (Drive, markdown, document conversion) |
...
+-- Drive
|   +-- ReadFile
|   +-- UpdateFile (uses FileVersionManager)
|   +-- ListFiles
|   +-- Search
+-- Markdown
|   +-- Edit
...
#### Memories (FTS + Tags + Structured Filters)
...
**Memory retrieval pattern** (replaces hybrid semantic/keyword search):
```sql
-- FTS ranked by relevance, weighted by importance and recency
SELECT id, content, tags, category, importance,
       ts_rank(search_vector, query) * importance AS weighted_rank
FROM memories, plainto_tsquery('english', $1) AS query
WHERE user_id = $2
  AND search_vector @@ query
  AND ($3::text[] IS NULL OR tags && $3)      -- optional tag filter
  AND ($4::varchar IS NULL OR category = $4)  -- optional category filter
ORDER BY weighted_rank DESC
LIMIT $5;
```

**Future extensibility**: Semantic vector search can be added later as an additive migration: add `embedding vector(1536)` column, create HNSW/IVFFlat index, backfill embeddings via Oban job. The `ContextBuilder` can then combine FTS rank + cosine similarity via Reciprocal Rank Fusion (RRF). No schema or API changes needed — purely additive.
...
**Decision**: PostgreSQL for all storage needs. Full-text search (tsvector/tsquery) + structured filters for memory retrieval. pgvector deferred to future migration.

**Rationale**:
- Ecto has excellent PostgreSQL support
- Oban uses PostgreSQL for job queuing (no Redis needed)
- Built-in full-text search (tsvector/GIN index) sufficient for MVP memory retrieval
...
- pgvector can be added later as additive migration (column + index + backfill) without schema changes
...
### ADR-006: Non-Destructive File Operations

**Status**: Accepted (non-negotiable requirement)

**Context**: User files must never be permanently deleted. Every modification must preserve the original.

**Decision**: Archive-before-replace pattern with full audit trail.

**Workflow**:
1. **PULL**: Download from Drive into Briefly temp directory, record checksum
2. **MANIPULATE**: Skill modifies file in temp workspace
3. **ARCHIVE**: Move original on Drive to `Archive/{folder_name}/{filename}_{ISO_timestamp}`, record archive location
4. **VERIFY**: Confirm archive succeeded, compare checksums
5. **REPLACE**: Upload new version to original Drive location
6. **RECORD**: Create `file_versions` entry + `file_operation_log` entries with complete audit trail

**Failure recovery**:
- Archive fails: Abort — original untouched, notify ops
- Replace fails: Restore from archive (archive location is recorded), notify ops
...
- Memory system (conversation persistence + FTS-based memory search with tags and structured filters)
...
| FTS relevance quality for memory | LOW | MEDIUM | FTS + importance weighting may miss conceptually related memories that lack keyword overlap. Mitigated by tags for explicit categorization. pgvector can be added later if retrieval quality is insufficient. |
...
2. **Memory retention**: How long should memories persist? Recommend decay-based system where importance decreases over time unless reinforced by access. Tags provide explicit categorization for structured retrieval alongside FTS.
