...
CREATE TABLE tasks (
  ...
  short_id VARCHAR(12) NOT NULL UNIQUE,   -- stable short reference for CLI/LLM (e.g., t_9X4K2M)
...
**Short task references (`short_id`)**: Every task gets a compact, stable reference (`t_` + 6-8 base32 chars). UUID remains the internal primary key for joins/integrity, while all CLI-facing task targeting uses `short_id` to reduce token usage and improve readability.
...
**CLI reference contract**: Skill inputs/outputs expose `task_ref`/`task_refs` (mapped to `tasks.short_id`), not raw UUIDs. Internal services resolve refs to UUIDs before query/update execution.
...
### 2.3 TaskRef Specification (`short_id`)

To keep CLI commands token-efficient and readable, task references follow a strict format.

**Canonical format**:
- Pattern: `t_[A-Z0-9]{7}`
- Example: `t_9X4K2M7`
- Character set: Crockford-style base32 subset without separators

**Generation rules**:
- Generate `short_id` at task creation time in application code (not user-supplied)
- Retry on uniqueness conflict until insert succeeds
- Never mutate `short_id` after creation

**Uniqueness and indexing**:
- Enforced by `UNIQUE` constraint on `tasks.short_id`
- Add btree index via the unique constraint (sufficient for point lookups)

**CLI contract**:
- Skills accept only `task_ref` / `task_refs` for task identity
- Skill outputs must include `task_ref` in list/detail views
- UUID is internal-only (joins, foreign keys, database integrity)

**Error semantics**:
- Unknown ref: `Task not found: t_XXXXXXX`
- Ambiguous input (only if partial refs are ever allowed): `Task ref is ambiguous; provide full task_ref`
- Invalid format: `Invalid task_ref format. Expected t_[A-Z0-9]{7}`
    ...
    %{
      ...
      parameters: %{
        ...
        "properties" => %{
          ...
          "parent_task_ref" => %{
            "type" => "string",
            "description" => "Short reference of parent task (e.g., t_9X4K2M) if this is a subtask. Optional."
          }
        },
        "required" => ["title"]
      }
    ...
    %{
      ...
      parameters: %{
        ...
        "properties" => %{
          "task_ref" => %{
            "type" => "string",
            "description" => "Short reference of the task to update (e.g., t_9X4K2M)."
          },
          "status" => %{
            "type" => "string",
            "enum" => ["todo", "in_progress", "blocked", "done", "cancelled"],
    ...
    %{
      ...
      parameters: %{
        ...
        "properties" => %{
          "task_ref" => %{
            "type" => "string",
            "description" => "Short reference of the task to archive (e.g., t_9X4K2M)."
          },
          "reason" => %{
            "type" => "string",
...
defmodule Assistant.Skills.Tasks.Get do
  ...
  def tool_definition do
    %{
      name: "tasks.get",
      description: "Get task details by short reference, multiple references, or recency. Use this for explicit retrieval (single task, known task set, or recent tasks) with optional expanded details.",
      parameters: %{
        "type" => "object",
        "properties" => %{
          "task_ref" => %{
            "type" => "string",
            "description" => "Short reference of a single task to retrieve (e.g., t_9X4K2M). Optional if task_refs or recent is provided."
          },
          "task_refs" => %{
            "type" => "array",
            "items" => %{"type" => "string"},
            "description" => "List of short task references to retrieve in one call. Optional."
          },
          "recent" => %{
            "type" => "boolean",
...
- **Single**: `tasks.get --task_ref t_9X4K2M`
- **Plural**: `tasks.get --task_refs t_9X4K2M,t_3M7Q1ZA,t_8KD2PWR`
...
`tasks.search` responses should include both `task_ref` and `title` in list output so follow-up commands can reference tasks cheaply (for example: `tasks.update --task_ref t_9X4K2M --status done`).
...
**B. Hybrid retrieval on relevant topics**:
When the user mentions something that relates to an existing task (e.g., "I need to work on the Q1 report"), the memory system should surface the relevant task entry using hybrid retrieval. Tasks' `search_vector` and any memory entries from task completions are retrievable via PostgreSQL FTS + pgvector similarity + structured filters.
