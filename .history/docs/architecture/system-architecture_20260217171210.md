...
|   |   |       +-- list.ex
|   |   |
|   |   +-- files/              # Content Sync + Versioning System
|   |   |   +-- sync_manager.ex     # SYNC/NORMALIZE/PUBLISH/ARCHIVE orchestration
|   |   |   +-- normalizer.ex       # Docs->MD, Sheets->CSV, Slides->MD+assets
|   |   |   +-- workspace.ex        # Temp directory management (Briefly)
|   |   |   +-- version.ex          # FileVersion schema
|   |   |
|   |   +-- memory/             # Memory System
|   |   |   +-- store.ex        # Conversation persistence
|   |   |   +-- search.ex       # Hybrid (FTS + pgvector) + structured filter queries
|   |   |   +-- context_builder.ex  # Assemble context for LLM (hybrid-ranked + importance)
...
- Google Drive client (service account via Goth) + synced content workflow
- Memory system (conversation persistence + hybrid memory search with FTS, pgvector, tags, and structured filters)
- Basic skills: files.read, files.write, files.search
...
- Advanced files domain capabilities (slides markdown+assets workflows, format-specific publish polish)
