...
defmodule Assistant.Integrations.OpenRouter do
    ...
    with {:ok, body} <- build_request_body(messages, opts) do
      case do_request(body, Keyword.get(opts, :api_key)) do
        ...
        {:ok, %{status: status, body: resp_body}} when status >= 400 ->
          error_message = error_message_from_body(resp_body) || "Unknown error"
        ...
        {:error, %Req.TransportError{reason: reason}} ->
          Logger.error("OpenRouter connection error: #{inspect(reason)}", reason: inspect(reason))
          {:error, {:connection_error, reason}}

        {:error, reason} ->
          Logger.error("OpenRouter request failed: #{inspect(reason)}", reason: inspect(reason))
  ...
  defp error_message_from_body(body) when is_map(body), do: get_in(body, ["error", "message"])
  defp error_message_from_body(body) when is_binary(body) and body != "", do: body
  defp error_message_from_body(_), do: nil
