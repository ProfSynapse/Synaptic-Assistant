...
**Dynamic enum population**: The `domain` enum is populated at startup from the skill registry. If registered domains are `[:email, :calendar, :files, :tasks, :hubspot, :memory]`, the enum becomes `["email", "calendar", "files", "tasks", "hubspot", "memory"]`. This gives the LLM a clear menu of domains without enumerating every skill.
...
- files (5 skills): Read, write, search, move, and sync managed content
...
Files:     files.read, files.write, files.search, files.move, files.sync
...
[6. Memory context (hybrid retrieval: FTS + pgvector + structured filters)]
Relevant memories:
...
| Memory context | ~200-500 | Per-turn, hybrid retrieval (FTS + pgvector + structured filters) |
...
#   {:domains} -> [:email, :calendar, :files, :tasks, :hubspot, :memory]
...
@domain_descriptions %{
  ...
  files: "Read, write, search, move, and sync managed content",
...
- tasks.get --task_ref t_9X4K2M | --task_refs t_9X4K2M,t_3M7Q1ZA | --recent true [--limit N]
- files.read --file_ref f_ABC123
- files.write --file_ref f_ABC123 --instructions "..."
- files.search --query "..." [--folder ...]
...
| Memory search | PostgreSQL hybrid retrieval (FTS + pgvector) + structured filters | Strong keyword + semantic recall in one datastore |
| Memory deduplication | Hybrid near-duplicate check (FTS + vector similarity) before insert | Prevents memory bloat from repeated saves of same information |
