...
| **Memory progressive disclosure** | Summary + linked transcript segment | Memory entries store both a compact summary AND links to the original conversation messages (`segment_start_message_id`, `segment_end_message_id` FKs into `messages` table). `memory.search` returns summaries for fast scanning; `memory.get` fetches the full raw transcript for that topic segment. Model can find the topic, then dive into exactly what was said. |
...
| `memory` | memory.save, memory.search, memory.get | Hybrid retrieval (FTS + pgvector + filters). `memory.get --ids=id1,id2,...` fetches full conversation transcripts for one or more topic segments (progressive disclosure — plural IDs supported). |
...
│   │   │       ├── search.ex   # Returns summaries (fast scan, low token cost)
│   │   │       ├── get.ex      # Fetches full transcript for one or more topic segments (progressive disclosure — accepts --ids=id1,id2,... for bulk fetch)
│   │   │       ├── save.ex
│   │   │       └── entities.ex
│   │   │
...
│   │       ├── memory_entry.ex # + tags TEXT[], search_text tsvector, segment_start_message_id UUID FK, segment_end_message_id UUID FK
│   │       ├── memory_entity.ex
